...
if __name__ == "__main__":
    ...
    if narg == 2:
        os.system("bwa mem -R '@RG\\tID:RD\\tSM:RD\\tLB:RD\\tPL:Illumina' %s %s > %s" % (
            reference, input1, outprefix + ".sam"))

    if narg == 3:
        os.system("bwa mem -R '@RG\\tID:RD\\tSM:RD\\tLB:RD\\tPL:Illumina' %s %s %s > %s" % (
            reference, input1, input2, outprefix + ".sam"))
    ...
    for RD in RDs:
        ...
        if percentage > info[RD][1]:
            prediction = 'P'
            resultMat.append(
                [RD, outStats[0], outStats[1], outStats[2], outStats[3], RD_length[RD], "%.2f" % percentage,
                 prediction])
        else:
            resultMat.append(
    ...
    outH.write(
        "# RD_name\tMaximum_depth\tMinimum_depth\tMedian_depth\t#pos_pass_cutoff\t#total_pos\t%pos_pass_cutoff\tPrediction\tInformation\n")
